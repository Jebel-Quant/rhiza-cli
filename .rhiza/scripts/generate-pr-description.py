...
def categorize_files(files: list[str]) -> dict[str, list[str]]:
    ...
    for filepath in files:
        ...
        if path_parts[0] == ".github":
            ...
            categories["Documentation"].append(filepath)
        elif filepath in ["Makefile", "ruff.toml", "pytest.ini", ".editorconfig", 
...
def get_last_sync_date(repo_path: Path) -> str | None:
    ...
    output = run_git_command(
        ["log", "--grep=rhiza", "--grep=Sync", "--grep=template", 
         "-i", "--format=%cI", "-1"],
        cwd=repo_path
    )
...
def generate_pr_description(repo_path: Path) -> str:
    ...
    lines.append("")
    lines.append(f"This PR synchronizes the repository with the [{template_repo}](https://github.com/{template_repo}) template.")
    lines.append("")
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate PR description for rhiza sync"
    )
    parser.add_argument(
        "repo_path",
        nargs="?",
