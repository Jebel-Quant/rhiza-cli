...
class TestSyncE2ETypicalWorkflow:
    ...
    def test_first_sync_copies_all_template_files(self, project, git_ctx, tmp_path):
        """First sync (no lock file) copies every template file into target."""
        ...
        with patch("rhiza.models._git.context.GitContext.clone_at_sha"):
            git_ctx.sync_merge(
                ...
                excludes=set(),
                lock=_make_lock("sha_v1", [str(p) for p in template_files]),
            )
    ...
    def test_subsequent_sync_applies_template_changes(self, mock_warn, project, git_ctx, tmp_path):
        ...
        with patch("rhiza.models._git.context.GitContext.clone_at_sha"):
            git_ctx.sync_merge(
                ...
                upstream_sha="sha_v1",
                base_sha=None,
                template_files=template_files_v1,
                template=RhizaTemplate(
                    template_repository="example/repo", include=["Makefile", "config.py", "README.md"]
                ),
                excludes=set(),
                lock=_make_lock("sha_v1", [str(p) for p in template_files_v1]),
            )
        ...
        template_files_v2 = [Path("Makefile"), Path("config.py"), Path("new.yml")]
        ...
        with patch("rhiza.models._git.context.GitContext.clone_at_sha", side_effect=populate_base):
            git_ctx.sync_merge(
                ...
                upstream_sha="sha_v2",
                base_sha="sha_v1",
                template_files=template_files_v2,
                template=RhizaTemplate(
                    template_repository="example/repo", include=["Makefile", "config.py", "new.yml"]
                ),
                excludes=set(),
                lock=_make_lock("sha_v2", [str(p) for p in template_files_v2]),
            )
...
class TestSyncE2EOrphanedFiles:
    ...
    def test_orphaned_files_removed_when_template_removes_a_file(self, mock_warn, project, git_ctx, tmp_path):
        ...
        template_files_v2 = [Path("file_a.txt")]
...
class TestSyncE2EExcludedFiles:
    ...
    def test_previously_tracked_file_excluded_is_not_removed(self, mock_warn, project, git_ctx, tmp_path):
        ...
        - Sync 2: file_b.txt is excluded (not in template_files), but it is
        ...
        template_files_v2 = [Path("file_a.txt")]  # file_b excluded from template_files
...
class TestSyncE2EUpdatedTemplateYml:
    ...
    def test_removing_include_entry_removes_orphaned_file(self, mock_warn, project, git_ctx, tmp_path):
        ...
        - Sync 2: ``file_b.txt`` is no longer in the template file set; orphan
